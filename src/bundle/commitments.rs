...
pub(crate) fn hash_bundle_txid_data<A: Authorization, V: Copy + Into<i64>>(
    ...
    for action in bundle.actions().iter() {
        ...
        ch.update(&action.encrypted_note().enc_ciphertext.as_ref()[..52]);

        mh.update(&action.encrypted_note().enc_ciphertext.as_ref()[52..564]);

        nh.update(&action.cv_net().to_bytes());
        nh.update(&<[u8; 32]>::from(action.rk()));
        nh.update(&action.encrypted_note().enc_ciphertext.as_ref()[564..]);
